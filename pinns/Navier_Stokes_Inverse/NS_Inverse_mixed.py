...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 27 12:27:45 2023
...
C1 = dde.Variable(np.float16(0.0), dtype=np.float16)
C2 = dde.Variable(np.float16(0.0), dtype=np.float16)
...
model.compile(optimizer = adam, lr=1e-3, external_trainable_variables=[C1, C2])
loss_history, train_state = model.train(
    iterations=10000, callbacks=[variable], display_every=10000, disregard_previous_best=True
)
# dde.saveplot(loss_history, train_state, issave=True, isplot=True)
model.compile(optimizer = adam, lr=1e-4, external_trainable_variables=[C1, C2])
loss_history, train_state = model.train(
