...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 26 14:07:06 2023
...
data_slice = np.loadtxt("newslice_data.txt").astype(np.float16)

# If error is high, set data_slice to np.loadtxt("newPDE_data.txt")

eta = 1000

data_slip = np.loadtxt("stenosis_wall_points.txt").astype(np.float16)

# Import all x,y,z points to use
PDE_data = np.loadtxt("newPDE_data.txt")[:, 0:3].astype(np.float16)
...
def PDE(t, y):
    ...
    momentum_x = (u * du_dx + v * du_dy + w * du_dz) + dp_dx - 1/Re * (du_dxx + du_dyy + du_dzz)
    momentum_y = (u * dv_dx + v * dv_dy + w * dv_dz) + dp_dy - 1/Re * (dv_dxx + dv_dyy + dv_dzz)
    momentum_z = (u * dw_dx + v * dw_dy + w * dw_dz) + dp_dz - 1/Re * (dw_dxx + dw_dyy + dw_dzz)
...
# For tf, this may need to be a numpy array of zeros (np.zeros)
zeros = np.zeros((slip.shape[0],1)).astype(np.float16)
noslip_1 = dde.PointSetBC(slip, zeros, component=0)
...
# net = dde.nn.FNN([3] + 2 * [20] + [4], "tanh", "Glorot normal")
...
adam = tf.keras.optimizers.Adam(epsilon=1e-2)
model.compile(optimizer = adam, lr=1e-3, loss_weights=[0, 0, 0, 0, 0, 0, 0, 1e2, 1e2, 1e2])
losshistory, train_state = model.train(iterations=10000, display_every = 10000)
model.compile(optimizer = adam, lr=1e-3, loss_weights=[1, 1, 1, 1, 1e2, 1e2, 1e2, 1e2, 1e2, 1e2])
losshistory, train_state = model.train(iterations=150000, display_every = 150000)
model.compile(optimizer = adam, lr=1e-4, loss_weights=[1, 1, 1, 1, 1e2, 1e2, 1e2, 1e2, 1e2, 1e2])
...
errors.to_csv("L2_Relative_Errors_Stenosis_Test_Case_tf_mp.csv")
tosave = np.hstack((X, y_pred[:, 0:3], y_pred[:, 3:4] + p_factor, gamma))

np.savetxt("networkoutput_stenosis_tf_mp.txt", tosave)
# calculate the peak memory used
